...
class EditorManager
{
  ...
  generateROM = () =>
  {
    ...
    try
    {
      const {romTypeMapKey, hackAuthor} = this;
      extras.successCallback = this.downloadGeneratedROM;
      extras.errorCallback = this.onActionResult;
      extras.errorMessage = "Problems generating the ROM!";
      extras.fileObject = editorManagerService.generateROM(romTypeMapKey, hackAuthor);
  ...
  downloadGeneratedROM = (extras) =>
  {
    ...
    const name = editorManagerService.getROMName(this.romTypeMapKey);
  ...
  setRomTypeMapKey = (romTypeMapKey) =>
  {
    this.romTypeMapKey = romTypeMapKey;
  }

  getViewData = () =>
  {
    const {componentData, hackAuthor, romTypeMapKey, generatingROM} = this;
    return {componentData, hackAuthor, romTypeMapKey, generatingROM};
  }
