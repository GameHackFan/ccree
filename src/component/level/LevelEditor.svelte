...
<div class="level flex-i-row">
  ...
  <span class={`window-text${data.componentData.showHintText ? "" : " hidden"}`}>
    ...
    you can change their X and Y positions, and you can 
    change the spawn delay time.
  </span>
  <span class={`window-text${data.componentData.showHintText ? "" : " hidden"}`}>
  ...
  <span class={`window-text${data.componentData.showHintText ? "" : " hidden"}`}>
    The Trigger Position and Spawn Delay Time always have
    a minimum and maximum value. The editor will allow 
  ...
  <div class={`enemy-group${data.showLevelEditArea ? "" : " hidden"}`}>
    <div class={`level-display${data.levelAreaScroll ? " vertical-scroll" : ""}`}>
