...
<div class="manager flex-i-row">
  ...
  <span class={`window-text${data.componentData.showHintText ? "" : " hidden"}`}>
    ...
    a hack like that with CCREE may work or it may result in a 
    bugged ROM, make sure you're editing the default Captre's ROM.
  </span>
  <span class={`window-text${data.componentData.showHintText ? "" : " hidden"}`}>
  ...
  <div class="window-content-line flex-i-col">
    ...
    <select
      class="button-solid"
      value={data.romTypeMapKey}
      on:change={(e) => editorManager.onValueChange(e, "setRomTypeMapKey")}
    >
      <option value="">Select a ROM Type</option>
      {#each editorManager.createROMTypeList() as rtd}
        <option value={rtd.key}>{rtd.label}</option>
      {/each}
    </select>
  </div>
  <div>
